...
{
    public class CreateSchemaField(IMongoRepository<SchemaContest> _mongoRepository, GetByIdSchema _getByIdSchema, IMongoRepository<Customer> _customerRepository, UserRequestContext _userRequest)
    {
        ...
        private async Task<Result<SchemaContest>> ValidateCatalog(SchemaContest schema, FieldRequest request)
        {
            ...
            var customer = await _customerRepository.FindByIdAsync(_userRequest.CustomerId);
            if (customer == null)
