...
using AgoraCertaminaBack.UseCases.Catalogs;
...
{
    public class GetDataSchemaById( IMongoRepository<SchemaContest> _mongoRepository, IMongoRepository<Customer> _customerRepository, UserRequestContext _userRequest)
    {
        public async Task<Result<DataSchemaDTO>> Execute(string schemaId)
        {
            var schema = await _mongoRepository.FindOneAsync(
                ...
                filter.CustomerId == _userRequest.CustomerId &&
                filter.IsActive);

            if (schema == null)
                return Result.NotFound<DataSchemaDTO>("Schema not found");

            var customer = await _customerRepository.FindOneAsync( filter => filter.Id == _userRequest.CustomerId);
            ...
            if (customer.StaticFields != null)
            {
                allFields.AddRange(customer.StaticFields.Where(f => f.IsActive));
            }
