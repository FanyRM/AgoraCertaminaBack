...
namespace AgoraCertaminaBack.UseCases.Contests
{
    public class UpdateContest(IMongoRepository<Contest> _mongoRepository, UserRequestContext _userRequest, IFileManager _fileManager, GetByIdContest _getByIdContest )
    {
        ...
        public async Task<Result<string>> Execute(string assetId, ContestUpdateRequest updateRequest)
        {
            var validated = ValidateUpdateRequest(updateRequest);
            if (!validated.Success)
                return Result.Failure<string>(validated.ToString());

            var fetchedContest = await _getByIdContest.Execute(assetId);
            ...
            var result = await UpdateAssetAsync(permissionChecked.Value, updateRequest);
        ...
        {
            return contest.CustomerId != _userRequest.CustomerId
                ? Result.Failure<Contest>("Uneditable asset")
                : contest.Success();
        }

        private async Task<Result<string>> UpdateAssetAsync(Contest contest, ContestUpdateRequest updateRequest)
        {
            try
            {
                var result = await UpdateFieldsAsync(contest, updateRequest.Fields)
                    .Bind(_ => UpdateTags(contest, updateRequest.Tags))
                    .Bind(_ => SaveAssetAsync(contest));
            ...
            {
                return Result.Failure<string>("Insufficient permissions to update asset");
            }
            catch (Exception ex)
            {
                return Result.Failure<string>($"Error updating asset: {ex.Message}");
            }
        }
        ...
        private static Result<string> UpdateTags(Contest contest, List<TagDTO>? tags)
        {
            if (tags == null)
        ...
        private async Task<Result<string>> SaveAssetAsync(Contest contest)
        {
            try
            {
            ...
            {
                return Result.Failure<string>($"Error saving asset: {ex.Message}");
            }
        }
